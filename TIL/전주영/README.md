...
- 컨텐츠 기반 필터링 + 협업 필터링
    - 데이터가 많이 없으니까, 컨텐츠 기반 필터링으로 우선 진행
    - 데이터 정제
...
- 회원관리  (회원가입, 로그인, 취향 분석)
- 여행그룹 (그룹 생성, 그룹원 추가, 그룹 참여, 그룹원 삭제, 그룹 나가기, 그룹 해제)
- 여행지 (상세데이터 불러오기)
- 경로 (경로 탐색, 티맵 연동, 경로 생성, 평가, 방문 여행지 체크)
- 추천 (그룹 경로 추천, 테마 추천)
...
- 챗봇
- 날씨
- 비용 정산 (영수증 사진 찍어서 그거 분석해서 데이터 넣는 것도 해야됨)

# 23.11.06

### 데이터셋 추가 정제 및 mysql test

# 23.11.08

### 시스템 아키텍처 설계 및 코랩 사용 CF 개발

# 23.11.16

### PPT, UCC 제작 및 아이디어 회의
